		...
		<c:choose>
			<c:when test="<%= portletURL != null %>">
				...
				for (int i = 0; i < assetTagNames.length; i++) {
					...
					if(paramName!= null && paramName.equals("tag")){
						newParamName = "tagname";
						mustReplace = true;
					}

					portletURL.setParameter(newParamName, assetTagNames[i]);
					String url = HtmlUtil.escape(portletURL.toString());
					if(mustReplace){
						url = url.replace("tagname","tag");
					}
				%>

					<a class="label label-lg label-secondary text-uppercase" href="<%= url %>"> <%= assetTagNames[i] %></a>
